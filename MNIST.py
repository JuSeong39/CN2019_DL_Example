...
for i in range(mnist.test.images.shape[0]) :
    test_input = mnist.test.images[i].reshape((1, -1))
    output = sess.run(predict, feed_dict={X:test_input})
    input_image = test_input.reshape((28, 28))* 255
    input_image = input_image.astype(np.uint8)
    cv2.imshow('test', input_image)
    answer = np.argmax(mnist.test.labels[i])
    print('answer : %d\tpredict : %d' % (answer, output))
    cv2.waitKey(0)
